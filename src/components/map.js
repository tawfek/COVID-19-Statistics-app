...
ReactTimeago= (datee)=>{
    const timeAgo = new TimeAgo(this.props.user.language);
...
_onViewportChange = viewport => this.props.setViewport(viewport);
  ...
  render() {
    ...
    const { user, MapConfig, country,setMapLoaded,data } = this.props;
    const { viewport, darkMode ,language} = user;
    const {mapboxApiAccessToken,styles,loaded} = MapConfig
    const {latitude, longitude,zoom} = viewport
    const {fetchedData} = data
    let mapStyle = darkMode ?styles.dark : styles.light;

//     if(this.mapRef!==null && loaded){
//         console.log(1) ;
//         this.mapRef.getMap().setLayoutProperty('place-label', 'text-field', ['get',`name_${language}`])
...
//   }
    ...
    function FlyMe(lat=defaultViewPort.latitude,lng=defaultViewPort.longitude,zoom=defaultViewPort.zoom,iso='',reload=true) {
        const FlyTo =    
         ...
         var set_tab = iso
            self.props.setViewport(FlyTo) ;
            if(!reload){
                // self.setState({activekey:set_tab,keyChanged:true})
    
              setTimeout(() => {
                // this.setState({viewport:FlyTo,keyChanged:false})
                self.props.setViewport(FlyTo) ;
    ...
    return (
      <ReactMapGL
        onLoad={() => {setMapLoaded(true); this.SetMapLang()}}
        {...viewport}
      ...
      >
        {MarkerDB.countries.map((country) => {
          const country_name_lan = user.language==="en" ? country.country.name : country.country.name_ar;
          const StateFetched = fetchedData.data 
              ...
              <Row className="country-popover">
                <Col sm={24} md={12}>
                  <Statistic
                    ...
                    suffix={
                      <Badge
                        count={Render_status.cases.new + " " + __("New")}
                      ></Badge>
                    }
                  />
                </Col>
                      ...
                      <Badge
                        count={
                          Render_status.deaths.new === null
                            ? 0
                            : Render_status.deaths.new + " " + __("New")
                        }
                      ></Badge>
                    }
                    value={
